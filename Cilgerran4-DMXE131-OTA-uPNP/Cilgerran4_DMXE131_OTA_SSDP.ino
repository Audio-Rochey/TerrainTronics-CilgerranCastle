...
/* 5/4/2025
Added more OTA functionality. Go to the same IP address, but port :81/update 
The username and password to overwrite it is admin letmein*/
...
// SSDP device name
//const char* DEVICE_NAME = "TerrainTronics Cilgerran Castle"; // This is what will pop up 
String ssdpDeviceName; // Will be filled in with the wifi hostname
...
void onOTAEnd(bool success) {
  Serial.println(success ? "OTA finished successfully!" : "OTA update failed!");
}
...
void setup() {
  ...
  // initialize GPIO
  pinMode(D1Ledout, OUTPUT); pinMode(D2Ledout, OUTPUT);
  pinMode(D3Ledout, OUTPUT); pinMode(D4Ledout, OUTPUT);
  pinMode(D5Ledout, OUTPUT); pinMode(D6Ledout, OUTPUT);
  digitalWrite(D1Ledout, LOW); digitalWrite(D2Ledout, LOW);
  digitalWrite(D3Ledout, LOW); digitalWrite(D4Ledout, LOW);
  digitalWrite(D5Ledout, LOW); digitalWrite(D6Ledout, LOW);
  pinMode(inv_VLEDenable, OUTPUT);
  digitalWrite(inv_VLEDenable, LOW);

  // OTA on port 81
  ohteeehserver.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(200, "text/plain", "Hi! ElegantOTA AsyncDemo.");
  });
  ElegantOTA.begin(&ohteeehserver);
  ...
  ssdpDeviceName = WiFi.hostname();


  // SSDP description endpoint
    ...
    String xml =
      ...
      "<root xmlns=\"urn:schemas-upnp-org:device-1-0\">"
        ...
        "<device>"
          ...
          "<friendlyName>" + ssdpDeviceName + "</friendlyName>"
          "<manufacturer>terraintronics.com</manufacturer>"
           "<manufacturerURL>http://terraintronics.com</manufacturerURL>"
           "<modelURL>http://" + WiFi.localIP().toString() + "</modelURL>"
           "<presentationURL>http://" + WiFi.localIP().toString() + "</presentationURL>"
          "<modelName>" + ssdpDeviceName + "</modelName>"
          "<UDN>uuid:" + WiFi.macAddress() + "</UDN>"
        "</device>"
      "</root>";
  ...
  String ipURL = "http://" + ip.toString();   // e.g. "http://192.168.1.42"

  // SSDP setup & start
  SSDP.setSchemaURL("description.xml");
  SSDP.setHTTPPort(80);
  SSDP.setName(ssdpDeviceName.c_str());
  SSDP.setManufacturer("terraintronics.com");
  SSDP.setManufacturerURL("http://terraintronics.com");
  SSDP.setModelName(ssdpDeviceName.c_str());                 // modelName tag
  SSDP.setModelURL(("http://" + WiFi.localIP().toString()).c_str());  // modelURL
  SSDP.setURL("/");                                // presentationURL (root)
  SSDP.setDeviceType("upnp:rootdevice");
  if (!SSDP.begin()) {
    Serial.println("⚠️ SSDP failed to start");
  } else {
